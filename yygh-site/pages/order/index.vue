        ...
        <div class="table-wrapper table">
          <el-table
            ...
            <el-table-column label="操作">
              <template slot-scope="scope">
                <el-button type="text" class="v-link highlight clickable selected" @click="show(scope.row.outTradeNo)">详情</el-button>
              </template>
            </el-table-column>
          </el-table>
        </div>
...
export default {
  ...
  methods: {
    ...
    findPatientList() {
      patientApi.getAllPatients().then(response => {
        this.patientList = response.data
      })
    },
    getStatusList() {
      orderInfoApi.getStatus().then(response => {
