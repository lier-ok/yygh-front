...
export default {
    data() {
        return {
            ...
            props: {
                ...
                const { level } = node
                //异步获取省市区
                dictApi.getChildById(level ? node.value : '86').then(response => {
    ...
    methods: {
        ...
        fetchDataById(id) {
            patientApi.getPatientById(id).then(response => {
        ...
        getDict() {
            dictApi.getChildByHosType("CertificatesType").then(response => {
        ...
        saveData() {
            ...
            this.submitBnt = '正在提交...'
                patientApi.addPatient(this.patient).then(response => {
